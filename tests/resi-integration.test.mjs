...
const env={WMS_INTEGRATION_ENABLED:'true',SUPABASE_URL:'https://testproject.supabase.co',SUPABASE_SERVER_KEY:'sb_secret_fixture',MINI_ERP_RESI_RPC:'read_resi',WMS_OUTBOX_RPC:'wms_integration_event',MOKA_ACCESS_TOKEN:'fixture-token',MOKA_OUTLET_MAP:'{"ST01":"42"}',WMS_ALLOW_EVENT_SUBMIT:'true'};
...
test('Gateway keeps secrets server-only, requires complete ERP snapshots and maps paginated Moka variants',async()=>{
 ...
 const status=await (await gateway(req('status'),env)).text();assert(!status.includes('fixture'));assert.equal((await gateway(req('resi'),env)).status,200);assert.equal(calls[0].options.headers.apikey,'sb_secret_fixture');assert(!calls[0].options.headers.Authorization);
